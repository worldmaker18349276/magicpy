...
from MagicPart.Features.BooleanTracing import outOf
...
class CommonCommand(object):
    ...
    def Activated(self):
        ...
        if any(isDerivedFrom(s, ("Part::Compound", Compound)) for s in sel):
            ...
            need_recompute = all(not isTouched(s) for s in sel) or P.autorecomp
    ...
    def Activated(self):
        ...
        if len(sel) > 1:
            if P.incmdline:
                ...
                for s in sel:
                    FreeCADGui.doCommand("_ftrs.append(MagicPart.transform(%s))"%ftrstr(s))
                if P.autorecomp:
            ...
            if P.incmdline:
                FreeCADGui.doCommand("_ftr = MagicPart.transform(%s)"%ftrstr(sel[0]))
                if need_recompute:
                    FreeCADGui.doCommand("MagicPart.recompute([_ftr])")
                if P.autosel:
                    FreeCADGui.doCommand("MagicPart.select([_ftr])")
...
class SliceCommand(object):
    ...
    def Activated(self):
        ...
        sel = ftrlist(FreeCADGui.Selection.getSelection()[1:])
        if target in sel:
            sel.remove(target)
